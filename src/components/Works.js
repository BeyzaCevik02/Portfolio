    ...
    const getModalImagesForProject = (project) => {
        switch (project.title) {
            case 'Final Schoolproject':
                return ['/assets/works/designs/mailing_api.png'];
            case 'Schoolproject <br /> Craft CMS V5':
                return ['/assets/works/showcase/craft.png'];
            case 'Schoolproject <br /> WordPress':
                if (project.image.includes('wordpressks')) {
                    return ['/assets/works/showcase/kss.png'];
                } else if (project.image.includes('wordpress')) {
                    return ['/assets/works/showcase/gg.png'];
                }
                break;
            case 'Schoolproject <br /> Vue.js':
                return ['/assets/works/showcase/vue.png'];
            case 'Schoolproject <br /> SCSS':
                return ['/assets/works/showcase/scss.png'];
            default:
                return [];
        }
    };
        ...
        <div className="works">
            ...
            <div className="works__cards">
                {projectsData.map((project, index) =>
                    // Controleer op de specifieke werkjes die niet aanklikbaar moeten zijn
                    (project.title === "Internship Hamofa Brand Builders Client Website" && 
                    (project.image.includes('neyens') || project.image.includes('multi'))) ? (
                        ...
                        // Alle andere werkjes blijven klikbaar (met link of onClick)
                        project.link ? (
                            <a
                                key={index}
                                href={project.link}
